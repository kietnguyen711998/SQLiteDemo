...
import com.example.sqlite.module.employee.EmployeeBean
...
class EmployeeDatabaseAdapter(context: Context){
    ...
    class DatabaseHelper(context: Context): SQLiteOpenHelper(context,DATABASE_NAME,null,DATABASE_VERSION) {
        ...
        override fun onCreate(db: SQLiteDatabase?) {
            val querry = "CREATE TABLE "+TABLE_NAME+" ("+ KEY_ID+" INTEGER PRIMARY KEY AUTOINCREMENT, " +
                    ""+KEY_NAME+" VARCHAR(255), "+ KEY_EMAIL+" TEXT, "+ KEY_AGE+" INTEGER);"
            db?.execSQL(querry)
            if(db!=null){
